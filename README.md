...
- **درخواست چت** — ارسال درخواست چت به کاربر خاص از طریق پروفایل عمومی (۲ سکه هزینه)؛ هویت و پروفایل درخواست‌دهنده تا قبل از زدنِ «مشاهده» توسط گیرنده فاش نمی‌شه، و بعد از مشاهده به فرستنده هم اطلاع داده می‌شه که درخواستش دیده شده
...
- **پاکسازی تاریخچه** — بعد از پایان هر چت، امکان پاکسازی کل تاریخچه چت بصورت دوطرفه می باشد 

- **گزارش گفتگو** — کاربران می‌تونن در حین چت یا بعد از پایان، گفتگو رو گزارش بدن تا توسط هوش مصنوعی بررسی بشه
...
- **ساخت اتاق** — اتاقِ چتِ گروهیِ دائمی با ظرفیتِ ۲ تا ۵ نفر و جنسیتِ مشخص (پسرونه/دخترونه/فرقی نداره)؛ ۲۰ سکه هزینه
...
- **رله‌ی چندنفره** — پیام‌ها بینِ همه‌ی اعضا رله می‌شن؛ حذف، ویرایش و چتِ امن مشابهِ چتِ ۱به۱ پشتیبانی می‌شن

- **مدیریتِ owner** — بستن/بازکردنِ اتاق برای عضوگیری، اخراجِ عضو، حذفِ پیامِ هرکسی، و حذفِ کاملِ اتاق (به‌همراه پیشنهادِ پاک‌سازیِ تاریخچه)

- **خروج** — عضوِ عادی می‌تونه اتاق رو ترک کنه؛ owner فقط با حذفِ کاملِ اتاق خارج می‌شه؛ اگه فقط owner باقی بمونه، اتاق خودکار حذف می‌شه
...
- **بازگشت خودکار سکه** — اگه چت کمتر از ۳ پیام داشته باشه (ناموفق)، جستجو لغو/تایم‌اوت بشه، درخواست چت رد بشه، یا درخواست چت ظرف ۲ دقیقه بی‌پاسخ بمونه، سکه برگشت داده می‌شه
