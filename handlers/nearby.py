...
توی هر شعاع حداکثر NEARBY_RESULT_LIMIT (۵۰) نفر از Postgres گرفته
می‌شن (مرتب‌شده بر اساسِ فاصله، برای اینکه دقیقاً نزدیک‌ترین‌ها باشن نه
هرچی)، ولی ترتیبِ نهاییِ نمایش بر اساسِ last_seenِ Redis (زودترین
آنلاینی اول) دوباره مرتب می‌شه، چون آنلاین‌بودن برای شروعِ گفتگو
مهم‌تر از چند دهم کیلومتر فرقِ فاصله‌ست. صفحه‌بندی (NEARBY_PAGE_SIZE
نفر در هر صفحه) روی همین لیستِ نهایی، تو پایتون انجام می‌شه — چون
مرتب‌سازی بر اساسِ last_seen (که تو Redis‌ه) با OFFSET/LIMIT خودِ
Postgres ممکن نیست.
"""
...
NEARBY_RESULT_LIMIT = 50  # حداکثرِ کلِ نتایج در هر شعاع
NEARBY_PAGE_SIZE = 20  # تعدادِ نمایش‌داده‌شده در هر صفحه
...
async def show_nearby_users(
    update: Update, context: ContextTypes.DEFAULT_TYPE, radius_km: int = DEFAULT_NEARBY_RADIUS_KM, page: int = 0
) -> None:
    """توی شعاعِ مشخص‌شده (کیلومتر) تا NEARBY_RESULT_LIMIT نفر می‌گیره،
    بر اساسِ زودترین آنلاینی (last_seenِ Redis) مرتب می‌کنه، و صفحه‌ی
    خواسته‌شده (هرکدوم NEARBY_PAGE_SIZE نفر) رو نشون می‌ده."""
    telegram_user = update.effective_user
    ...
    async with async_session() as session:
        ...
        stmt = (
            ...
            .where(
                ...
                ST_DWithin(User.location, me.location, radius_km * 1000),
            )
            .order_by(distance_col.asc())
            .limit(NEARBY_RESULT_LIMIT)
        )
        result = await session.execute(stmt)
        candidates = result.all()  # list of (User, distance_m), حداکثر ۵۰ تا، فعلاً بر اساسِ فاصله مرتب

    if not candidates:
        text = f"فعلاً کسی توی {_to_fa(radius_km)} کیلومتری‌ت پیدا نشد. بعداً دوباره امتحان کن."
        keyboard = nearby_keyboard(True, radius_km=radius_km, page=0, has_next=False)
    else:
        last_seen_map = await rc.get_last_seen_many([c[0].id for c in candidates])
        # ترتیبِ نهایی: زودترین آنلاینی اول (کسی که last_seen نداره، آخرِ همه)
        candidates.sort(key=lambda c: last_seen_map.get(c[0].id, -1), reverse=True)

        total = len(candidates)
        start = page * NEARBY_PAGE_SIZE
        end = min(start + NEARBY_PAGE_SIZE, total)
        page_candidates = candidates[start:end]

        lines = [f"📍 کاربرانِ توی {_to_fa(radius_km)} کیلومتری‌ت ({_to_fa(total)} نفر):\n"]
        for candidate, distance_m in page_candidates:
            name = candidate.display_name or "کاربر ناشناس"
            profile_line = f"/user_{candidate.referral_code}" if candidate.referral_code else "—"
            online_label = rc.format_last_seen(last_seen_map.get(candidate.id))
            lines.append(
                f"• {name} ({profile_line})\n"
                f"  📏 تقریباً {distance_m / 1000:.1f} کیلومتر — {online_label}"
            )
        text = "\n".join(lines)
        keyboard = nearby_keyboard(True, radius_km=radius_km, page=page, has_next=end < total)
