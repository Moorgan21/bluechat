"""
هندلرهای بخش «گزارش کاربر» — کاربر می‌تونه همراه گفتگوی فعلی رو گزارش
بده. گزارش در Postgres ثبت می‌شه و بلافاصله توسط سیستم قضاوتِ AI
(judge.py، بر پایه‌ی Gemini) بر اساس تاریخچه‌ی متنیِ گفتگو بررسی می‌شه:

    - اگه AI تشخیص بده گزارش‌شده واقعاً مقصر بوده → یه اخطار می‌گیره
      (با ذکر دلیل و شماره‌ی اخطار، مثلاً «۱ از ۵»)؛ در اخطار پنجم
      خودکار بن می‌شه.
    - اگه AI تشخیص بده گزارش نادرست/بی‌اساس بوده → گزارش‌دهنده یه
      اخطار می‌گیره.
    - اگه تاریخچه‌ی گفتگو قبلاً (با تایید دوطرفه) پاک شده باشه، هیچ
