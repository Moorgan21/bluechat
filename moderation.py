...
"""بررسی محتوای تصویر با Gemini Vision، برای جلوگیری از ثبت عکس‌های
نامناسب به‌عنوان عکس پروفایل.

نیاز به `pip install google-genai` و env var به اسم GEMINI_API_KEY.

این یه لایه‌ی کمکیه، نه سیستم خطاناپذیر - مدل ممکنه گاهی اشتباه کنه
(هم رد کردن عکس سالم، هم تایید عکس نامناسب). برای ترافیک واقعی بهتره
کنارِ گزارش‌دهیِ کاربران (handlers/report.py) و بازبینی انسانی استفاده
بشه، نه تنها.

عکس به‌صورت بایت میره برای مدل multimodal Gemini و ازش یه JSON با
safe/unsafe و دلیل می‌خوایم. اگه ارزیابی به هر دلیلی شکست بخوره
(خطای شبکه، پاسخ نامعتبر...) عکس رد می‌شه، نه تایید - محتوای
بررسی‌نشده نباید منتشر بشه.
"""
