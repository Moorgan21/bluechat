...
"""قضاوت گزارش‌های پروفایل با Gemini Vision. کسی که پروفایلِ یکی رو
گزارش می‌ده (عکس/نام/بیو نامناسب)، عکس و متنِ اون پروفایل می‌ره برای
Gemini تا تشخیص بده واقعاً مشکلی داره یا نه.

اینجا Gemini لازمه چون نیاز به تحلیل تصویره و DeepSeek این قابلیتو
نداره (برای متنِ گزارشِ گفتگو تو judge.py از DeepSeek استفاده می‌کنیم
چون ارزون‌تره، ولی برای عکس چاره‌ای نیست).

guilty یعنی پروفایل واقعاً مشکل داره: کاربر بلافاصله بلاک می‌شه
(is_banned=True، جدا از شمارشِ اخطارها) و گزارش‌دهنده ۵ سکه می‌گیره.
dismissed یعنی گزارش الکی بوده و این‌بار گزارش‌دهنده اخطار می‌گیره.

هر خطایی (شبکه، پاسخ نامعتبر) پیش بیاد، هیچ اقدامی انجام نمی‌شه؛ نه
بن، نه اخطار، نه پاداش.
"""
...
async def _run_gemini_profile_judge(
    ...
    # json.dumps می‌کنیم تا نام/بیو نتونن با newline یا کاراکتر خاص از
    # فیلد متنیشون فرار کنن و پرامپت رو به‌هم بریزن، مثل همون حفاظی که
    # judge.py با JSON Lines برای transcript داره.
