...
async def add_recent_partners(user_a: int, user_b: int) -> None:
    """بعد از هر matching موفق صدا زده می‌شه — هر دو طرف رو به لیست اخیرِ
    همدیگه اضافه می‌کنه تا ۲ ساعت دوباره match نشن."""
    await r.set(KEY_RECENT_PARTNER.format(user_id=user_a, partner_id=user_b), "1", ex=TTL_RECENT_PARTNERS)
    await r.set(KEY_RECENT_PARTNER.format(user_id=user_b, partner_id=user_a), "1", ex=TTL_RECENT_PARTNERS)
...
async def pop_matching_waiting(user_id: int, desired_gender: Optional[str]) -> Optional[int]:
    """برای user_id (که خودش desired_gender رو مشخص کرده) یه کاندیدای
    مناسب از صف(ها) پیدا می‌کنه:
    - اگه desired_gender مشخص شده، فقط توی صفِ همون جنسیت می‌گرده.
    - اگه None باشه (فرقی نمی‌کنه)، توی هر دو صف می‌گرده.
    از بین کاندیداها، فقط اونایی در نظر گرفته می‌شن که ترجیح خودشون
    (desired_gender خودشون) با جنسیتِ user_id سازگار باشه (یا خودشون
    هم فرقی‌نمی‌کنه انتخاب کرده باشن) — یعنی تطابق دوطرفه."""
    own_gender = await _user_gender_str(user_id)
